<?php
/**
 * Apache2 VirtualHost Manager v2.3
 * ─────────────────────────────────
 * INSTALLATION:
 *   sudo cp index.php /var/www/html/index.php
 *
 * ARCHITECTURE v3 : toutes les opérations privilégiées (a2ensite, a2dissite,
 * apachectl, écriture/suppression des .conf, /etc/hosts) passent par le
 * daemon root vhost-helper via la socket /run/vhost-manager.sock.
 * → Aucun sudoers ni chown de sites-available n'est nécessaire.
 *
 * INSTALLATION DU DAEMON :
 *   sudo cp vhost-helper /usr/local/bin/vhost-helper
 *   sudo chmod 755 /usr/local/bin/vhost-helper
 *   sudo cp vhost-helper.service /etc/systemd/system/
 *   sudo systemctl daemon-reload
 *   sudo systemctl enable --now vhost-helper
 */
...
function parseVhostFile(string $file): array {
    ...
    $v = [
        ...
        'doc_root'=>'', 'port'=>'80', 'ssl'=>false,
        'admin_email'=>'', 'raw'=>$content,
    ];
...
function parseHostsFile(): array {
    ...
    foreach ($lines as $i => $line) {
        ...
        if ($trimmed === '') { $entries[] = $entry; continue; }
        if (str_starts_with($trimmed,'#')) {
            $entry['type']    = 'comment';
            $entry['comment'] = $trimmed;
            $entries[] = $entry; continue;
        }
...
// ─── ACTIONS ───────────────────────────────────────────────────────────────
$message = null; $msgType = 'info';

// GET: view raw config
if (isset($_GET['view'])) {
    $f = SITES_AVAILABLE.'/'.sanitizeFilename(basename($_GET['view']));
    if (file_exists($f)) { header('Content-Type: text/plain'); echo @file_get_contents($f); exit; }
}
...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    switch ($action) {
        ...
        case 'disable':
            ...
            if ($res['code']===0) { runVhostCmd('apache_reload'); $message="✓ Site <strong>$site</strong> ".($action==='enable'?'activé':'désactivé')."."; $msgType='success'; }
            else { $message="✗ ".htmlspecialchars($res['output']); $msgType='error'; }
            break;
        ...
        case 'create':
            ...
            if (file_exists($fpath)) { $message="✗ Le fichier <strong>$fname</strong> existe déjà."; $msgType='error'; break; }
            $content = !empty($_POST['raw_mode']) ? ($_POST['raw_content']??'') : generateConf($_POST);
            $res = runVhostCmd('write_conf', [$fname], $content);
            if ($res['code']===0) {
                $message="✓ VHost <strong>$fname</strong> créé. Activez-le pour l'utiliser."; $msgType='success';
            } else { $message="✗ ".htmlspecialchars($res['output']); $msgType='error'; }
            break;
        ...
        case 'edit_save':
            ...
            if (!file_exists($fpath)) { $message="✗ Fichier introuvable."; $msgType='error'; break; }
            $res = runVhostCmd('write_conf', [$site], $content);
            if ($res['code']===0) {
                runVhostCmd('apache_reload');
                $message="✓ <strong>$site</strong> sauvegardé et Apache rechargé."; $msgType='success';
            } else { $message="✗ ".htmlspecialchars($res['output']); $msgType='error'; }
            break;
        ...
        case 'delete':
            $fpath = SITES_AVAILABLE.'/'.$site;
            if (!file_exists($fpath)) { $message="✗ Fichier introuvable."; $msgType='error'; break; }
            if (isEnabled($site)) { runVhostCmd('a2dissite', [$site]); runVhostCmd('apache_reload'); }
            $res = runVhostCmd('delete_conf', [$site]);
            if ($res['code']===0) { $message="✓ VHost <strong>$site</strong> supprimé."; $msgType='success'; }
            else { $message="✗ ".htmlspecialchars($res['output']); $msgType='error'; }
            break;
        ...
        case 'hosts_add':
            ...
            if (!$ip || !$hosts) { $message="✗ IP et nom d'hôte obligatoires."; $msgType='error'; break; }
            if (!filter_var($ip, FILTER_VALIDATE_IP)) { $message="✗ Adresse IP invalide : <strong>".htmlspecialchars($ip)."</strong>"; $msgType='error'; break; }
            ...
            foreach ($hostList as $h) {
                if (hostsEntryExists($entries, $ip, $h)) {
                    $message="✗ L'entrée <strong>$ip $h</strong> existe déjà."; $msgType='error'; break 2;
                }
            }
            $newEntry = ['idx'=>count($entries), 'raw'=>'', 'type'=>'host',
                         'ip'=>$ip, 'hosts'=>$hostList,
                         'inline_comment'=>$comment ? "# $comment" : ''];
            $entries[] = $newEntry;
            if (writeHostsFile($entries)) { $message="✓ Entrée <strong>$ip ".implode(' ',$hostList)."</strong> ajoutée."; $msgType='success'; }
            else { $message="✗ Impossible d'écrire ".HOSTS_FILE." — vérifiez que le daemon vhost-helper tourne."; $msgType='error'; }
            break;
        ...
        case 'hosts_edit':
            ...
            $hosts   = trim($_POST['h_hosts']    ?? '');
            $comment = trim($_POST['h_comment']  ?? '');
            if (!$ip || !$hosts || $idx < 0) { $message="✗ Données invalides."; $msgType='error'; break; }
            if (!filter_var($ip, FILTER_VALIDATE_IP)) { $message="✗ Adresse IP invalide."; $msgType='error'; break; }
            $hostList = preg_split('/[\s,]+/', $hosts, -1, PREG_SPLIT_NO_EMPTY);
            $entries  = parseHostsFile();
            if (!isset($entries[$idx])) { $message="✗ Entrée introuvable."; $msgType='error'; break; }
            $entries[$idx]['ip']             = $ip;
            $entries[$idx]['hosts']          = $hostList;
            $entries[$idx]['inline_comment'] = $comment ? "# $comment" : '';
            $entries[$idx]['type']           = 'host';
            if (writeHostsFile($entries)) { $message="✓ Entrée modifiée."; $msgType='success'; }
            else { $message="✗ Impossible d'écrire ".HOSTS_FILE."."; $msgType='error'; }
            break;

        case 'hosts_delete':
            $idx     = (int)($_POST['h_idx'] ?? -1);
            $entries = parseHostsFile();
            if ($idx < 0 || !isset($entries[$idx])) { $message="✗ Entrée introuvable."; $msgType='error'; break; }
            $removed = $entries[$idx]['ip'].' '.implode(' ',$entries[$idx]['hosts']);
            array_splice($entries, $idx, 1);
            // Réindexer
            foreach ($entries as $k => &$e) $e['idx'] = $k;
            if (writeHostsFile($entries)) { $message="✓ Entrée <strong>".htmlspecialchars($removed)."</strong> supprimée."; $msgType='success'; }
            else { $message="✗ Impossible d'écrire ".HOSTS_FILE."."; $msgType='error'; }
            break;
        ...
        case 'hosts_raw_save':
            ...
            if (strpos($content,'localhost') === false) {
                $message="✗ Refus : 'localhost' absent — entrées système obligatoires."; $msgType='error'; break;
            }
            $res = runVhostCmd('write_hosts', [], $content);
            if ($res['code']===0) { $message="✓ ".HOSTS_FILE." sauvegardé."; $msgType='success'; }
            else { $message="✗ ".htmlspecialchars($res['output']); $msgType='error'; }
            break;
        ...
        case 'hosts_toggle_comment':
            ...
            if ($e['type'] === 'host') {
                ...
                $e['type'] = 'comment'; $e['comment'] = $e['raw'];
                $message = "✓ Entrée commentée (désactivée)."; $msgType='success';
            } elseif ($e['type'] === 'comment' && preg_match('/^#\s*([\d:.]+)\s+(\S.*)/', $e['comment'], $cm)) {
                // Décommenter
                $parts = preg_split('/\s+/', trim($cm[2]));
                $e['type'] = 'host'; $e['ip'] = $cm[1]; $e['hosts'] = $parts;
                $e['inline_comment'] = ''; $e['raw'] = '';
                $message = "✓ Entrée décommentée (activée)."; $msgType='success';
            } else { $message="✗ Impossible de basculer ce type de ligne."; $msgType='error'; break; }
            if (!writeHostsFile($entries)) { $message="✗ Impossible d'écrire ".HOSTS_FILE."."; $msgType='error'; }
            break;
    }
...
if (isset($_GET['msg'])&&$_GET['msg']) { $message=urldecode($_GET['msg']); $msgType=$_GET['type']??'info'; }
...
$themes = [
    'carbon' => [
        'label'=>'Carbon','emoji'=>'🖤',
        '--bg'=>'#0d0f12','--bg2'=>'#131619','--bg3'=>'#1a1e23',
        '--border'=>'#252a30','--border2'=>'#2e3540',
        '--accent'=>'#00e5a0','--accent2'=>'#00b87a',
        '--red'=>'#ff4d6d','--yellow'=>'#ffd166','--blue'=>'#4da6ff',
        '--text'=>'#e2e8f0','--text2'=>'#8a96a3','--text3'=>'#505a65',
    ...
    'frost' => [
        ...
        '--border'=>'#d1dae6','--border2'=>'#b8c6d6',
        '--accent'=>'#0066cc','--accent2'=>'#0052a3',
        '--red'=>'#cc2244','--yellow'=>'#c07800','--blue'=>'#0066cc',
        '--text'=>'#1a2433','--text2'=>'#4a6080','--text3'=>'#8899aa',
        '--font-ui'=>"'Plus Jakarta Sans', sans-serif",'--font-mono'=>"'IBM Plex Mono', monospace",
    ...
    'amber' => [
        'label'=>'Amber','emoji'=>'🟡',
        '--bg'=>'#0f0a00','--bg2'=>'#1a1100','--bg3'=>'#221800',
        '--border'=>'#2e2000','--border2'=>'#3d2c00',
        '--accent'=>'#ffb300','--accent2'=>'#cc8f00',
        '--red'=>'#ff4422','--yellow'=>'#ffb300','--blue'=>'#66aaff',
        '--text'=>'#ffe580','--text2'=>'#997a00','--text3'=>'#554400',
        '--font-ui'=>"'VT323', monospace",'--font-mono'=>"'VT323', monospace",
    ...
    'dracula' => [
        ...
        '--accent'=>'#bd93f9','--accent2'=>'#9d73d9',
        '--red'=>'#ff5555','--yellow'=>'#f1fa8c','--blue'=>'#8be9fd',
        '--text'=>'#f8f8f2','--text2'=>'#9090c0','--text3'=>'#555580',
        '--font-ui'=>"'Outfit', sans-serif",'--font-mono'=>"'Fira Code', monospace",
    ...
    'nord' => [
        ...
        '--font-ui'=>"'Nunito', sans-serif",'--font-mono'=>"'JetBrains Mono', monospace",
...
body{background:var(--bg);color:var(--text);font-family:var(--font-ui);min-height:100vh;line-height:1.6;transition:background .3s,color .3s}
...
/* ── HEADER ── */
header{display:flex;align-items:center;justify-content:space-between;padding:26px 0 22px;border-bottom:1px solid var(--border);margin-bottom:28px;gap:14px;flex-wrap:wrap}
.logo{display:flex;align-items:center;gap:12px}
.logo-icon{width:42px;height:42px;border-radius:10px;background:linear-gradient(135deg,var(--accent),var(--accent2));display:flex;align-items:center;justify-content:center;font-size:18px;box-shadow:0 0 20px color-mix(in srgb,var(--accent) 40%,transparent)}
.logo h1{font-size:1.3rem;font-weight:800;letter-spacing:-.02em}
.logo span{font-family:var(--font-mono);font-size:.68rem;color:var(--text2)}
.header-right{display:flex;align-items:center;gap:10px;flex-wrap:wrap}

/* STATUS BADGE */
.status-badge{display:flex;align-items:center;gap:7px;padding:7px 13px;border-radius:999px;font-family:var(--font-mono);font-size:.7rem;border:1px solid var(--border2);background:var(--bg2)}
...
/* THEME SWITCHER */
.theme-switcher{display:flex;gap:5px;align-items:center;background:var(--bg2);border:1px solid var(--border2);border-radius:999px;padding:4px 8px}
.theme-btn{width:22px;height:22px;border-radius:50%;border:2px solid transparent;cursor:pointer;transition:transform .15s,border-color .15s;text-decoration:none;display:flex;align-items:center;justify-content:center;font-size:.75rem;background:transparent}
.theme-btn:hover{transform:scale(1.2)}
.theme-btn.active{border-color:var(--accent)}
.theme-swatch-carbon {background:#00e5a0}
.theme-swatch-frost  {background:#0066cc}
.theme-swatch-amber  {background:#ffb300}
.theme-swatch-dracula{background:#bd93f9}
.theme-swatch-nord   {background:#88c0d0}

/* BUTTONS */
.btn{display:inline-flex;align-items:center;gap:6px;padding:8px 15px;border-radius:8px;border:none;cursor:pointer;font-family:var(--font-ui);font-size:.82rem;font-weight:600;transition:all .15s;text-decoration:none;white-space:nowrap}
...
.btn-primary{background:var(--accent);color:<?= $theme==='frost' ? '#fff' : '#000' ?>;font-weight:700}
.btn-primary:hover{background:var(--accent2);box-shadow:0 0 16px color-mix(in srgb,var(--accent) 35%,transparent)}
...
.btn-success{background:color-mix(in srgb,var(--accent) 10%,transparent);color:var(--accent);border:1px solid color-mix(in srgb,var(--accent) 25%,transparent)}
.btn-success:hover{background:color-mix(in srgb,var(--accent) 20%,transparent)}
.btn-warning{background:color-mix(in srgb,var(--yellow) 10%,transparent);color:var(--yellow);border:1px solid color-mix(in srgb,var(--yellow) 25%,transparent)}
.btn-warning:hover{background:color-mix(in srgb,var(--yellow) 20%,transparent)}
...
.stat.red::before{background:var(--red)}.stat.blue::before{background:var(--blue)}.stat.yellow::before{background:var(--yellow)}
.stat-label{font-size:.68rem;color:var(--text2);text-transform:uppercase;letter-spacing:.08em;margin-bottom:5px}
.stat-value{font-size:1.8rem;font-weight:800;line-height:1}
.stat-sub{font-family:var(--font-mono);font-size:.65rem;color:var(--text3);margin-top:3px}

/* TOAST */
.toast{padding:13px 17px;border-radius:var(--radius);margin-bottom:22px;font-size:.86rem;display:flex;align-items:center;gap:10px;animation:slideIn .3s ease;border:1px solid transparent}
.toast.success{background:color-mix(in srgb,var(--accent) 10%,transparent);border-color:color-mix(in srgb,var(--accent) 30%,transparent);color:var(--accent)}
.toast.error{background:color-mix(in srgb,var(--red) 10%,transparent);border-color:color-mix(in srgb,var(--red) 30%,transparent);color:var(--red)}
.toast.info{background:color-mix(in srgb,var(--blue) 10%,transparent);border-color:color-mix(in srgb,var(--blue) 30%,transparent);color:var(--blue)}
...
.search-box input{width:100%;background:var(--bg2);border:1px solid var(--border2);color:var(--text);font-family:var(--font-mono);font-size:.8rem;padding:8px 10px 8px 36px;border-radius:8px;outline:none;transition:border-color .15s}
.search-box input:focus{border-color:var(--accent)}
.search-ico{position:absolute;left:11px;top:50%;transform:translateY(-50%);color:var(--text3);font-size:.85rem;pointer-events:none}
.filters{display:flex;gap:5px;flex-wrap:wrap}
.ftab{padding:6px 13px;border-radius:7px;font-size:.76rem;font-weight:600;cursor:pointer;border:1px solid var(--border2);background:transparent;color:var(--text2);transition:all .15s;font-family:var(--font-ui)}
.ftab.active,.ftab:hover{background:var(--accent);color:<?= $theme==='frost' ? '#fff' : '#000' ?>;border-color:var(--accent)}

/* VHOST CARDS */
.vhost-grid{display:flex;flex-direction:column;gap:9px}
.vhost-card{background:var(--bg2);border:1px solid var(--border);border-radius:var(--radius);overflow:hidden;transition:border-color .2s,transform .15s;animation:fadeUp .3s ease both}
.vhost-card:hover{border-color:var(--border2);transform:translateY(-1px)}
.vhost-card.disabled{opacity:.6}
@keyframes fadeUp{from{opacity:0;transform:translateY(8px)}}
.vhost-row{display:grid;grid-template-columns:28px 1fr auto auto;align-items:center;gap:14px;padding:15px 18px;cursor:pointer}
.vind{width:8px;height:8px;border-radius:50%;background:var(--text3);justify-self:center}
.vind.on{background:var(--accent);box-shadow:0 0 7px var(--accent)}
.vinfo{min-width:0}
.vname{font-weight:700;font-size:.92rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.vname a{color:var(--text);text-decoration:none;transition:color .15s}
.vname a:hover{color:var(--accent)}
.vmeta{font-family:var(--font-mono);font-size:.68rem;color:var(--text2);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-top:2px}
.tags{display:flex;gap:5px;align-items:center;flex-wrap:wrap}
.tag{display:inline-flex;align-items:center;gap:3px;padding:2px 7px;border-radius:5px;font-size:.65rem;font-family:var(--font-mono);font-weight:700}
.tag-ssl{background:color-mix(in srgb,var(--blue) 12%,transparent);color:var(--blue);border:1px solid color-mix(in srgb,var(--blue) 20%,transparent)}
.tag-on{background:color-mix(in srgb,var(--accent) 10%,transparent);color:var(--accent);border:1px solid color-mix(in srgb,var(--accent) 20%,transparent)}
.tag-off{background:color-mix(in srgb,var(--text3) 10%,transparent);color:var(--text3);border:1px solid var(--border)}
.tag-port{background:color-mix(in srgb,var(--yellow) 8%,transparent);color:var(--yellow);border:1px solid color-mix(in srgb,var(--yellow) 15%,transparent)}
.vactions{display:flex;gap:6px;align-items:center}
.chev{color:var(--text3);transition:transform .2s;font-size:.75rem;user-select:none}

/* DETAILS */
.vdetails{border-top:1px solid var(--border);padding:16px 18px 16px 60px;background:var(--bg);display:none}
.vdetails.open{display:block}
.detail-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:10px}
.di label{font-size:.65rem;text-transform:uppercase;letter-spacing:.08em;color:var(--text3);display:block;margin-bottom:2px}
.di .val{font-family:var(--font-mono);font-size:.78rem;color:var(--text);word-break:break-all}
.di .val a{color:var(--accent)}

/* MODAL */
.overlay{display:none;position:fixed;inset:0;z-index:100;background:rgba(0,0,0,.7);backdrop-filter:blur(5px);align-items:center;justify-content:center;padding:20px}
.overlay.open{display:flex}
.modal{background:var(--bg2);border:1px solid var(--border2);border-radius:14px;width:100%;max-width:780px;max-height:90vh;display:flex;flex-direction:column;overflow:hidden;animation:scaleIn .2s ease}
...
.form-group label{font-size:.72rem;text-transform:uppercase;letter-spacing:.07em;color:var(--text2);font-weight:600}
.form-group input,.form-group select,.form-group textarea{background:var(--bg);border:1px solid var(--border2);color:var(--text);font-family:var(--font-mono);font-size:.82rem;padding:9px 12px;border-radius:8px;outline:none;transition:border-color .15s;width:100%}
.form-group input:focus,.form-group select:focus,.form-group textarea:focus{border-color:var(--accent)}
.form-group textarea{resize:vertical;min-height:100px;line-height:1.6}
.form-group select option{background:var(--bg2)}
.form-hint{font-size:.68rem;color:var(--text3);margin-top:2px}
.toggle-row{display:flex;align-items:center;gap:10px;padding:10px 0}
.toggle-row label{font-size:.82rem;font-weight:600;cursor:pointer}
input[type=checkbox]{accent-color:var(--accent);width:16px;height:16px;cursor:pointer}
.raw-code{font-family:var(--font-mono);font-size:.78rem;color:var(--accent);background:var(--bg);border:1px solid var(--border2);border-radius:8px;padding:16px;white-space:pre-wrap;word-break:break-all;line-height:1.7;width:100%;outline:none;resize:vertical;min-height:300px}
.mode-tabs{display:flex;gap:0;margin-bottom:16px;border:1px solid var(--border2);border-radius:8px;overflow:hidden}
.mode-tab{flex:1;padding:9px;text-align:center;cursor:pointer;font-size:.8rem;font-weight:600;background:transparent;border:none;color:var(--text2);font-family:var(--font-ui);transition:all .15s}
.mode-tab.active{background:var(--accent);color:<?= $theme==='frost' ? '#fff' : '#000' ?>}
.sep{height:1px;background:var(--border);margin:16px 0}
.danger-zone{background:color-mix(in srgb,var(--red) 5%,transparent);border:1px solid color-mix(in srgb,var(--red) 20%,transparent);border-radius:var(--radius);padding:16px;margin-top:8px}
.danger-zone h3{font-size:.82rem;color:var(--red);margin-bottom:6px}
.danger-zone p{font-size:.78rem;color:var(--text2);margin-bottom:12px}

/* EMPTY */
.empty{text-align:center;padding:60px 24px;border:1px dashed var(--border2);border-radius:var(--radius);color:var(--text2)}
...
.tab-nav-item{padding:11px 22px;font-weight:700;font-size:.86rem;cursor:pointer;border-bottom:2px solid transparent;margin-bottom:-2px;transition:all .15s;text-decoration:none;color:var(--text2);display:flex;align-items:center;gap:7px}
.tab-nav-item:hover{color:var(--text)}
.tab-nav-item.active{color:var(--accent);border-bottom-color:var(--accent)}
.tab-badge{background:var(--accent);color:#000;border-radius:999px;padding:1px 7px;font-size:.65rem;font-weight:800}
.tab-pane{display:none}.tab-pane.active{display:block}

/* HOSTS TABLE */
.htable{width:100%;border-collapse:collapse;font-size:.82rem}
.htable thead tr{border-bottom:2px solid var(--border2)}
.htable th{padding:9px 12px;text-align:left;font-size:.68rem;text-transform:uppercase;letter-spacing:.08em;color:var(--text3);font-weight:700;white-space:nowrap}
.htable tbody tr{border-bottom:1px solid var(--border);transition:background .1s}
.htable tbody tr:hover{background:color-mix(in srgb,var(--accent) 4%,transparent)}
.htable tbody tr.is-comment{opacity:.45}
.htable tbody tr.is-comment:hover{background:color-mix(in srgb,var(--yellow) 4%,transparent)}
.htable td{padding:9px 12px;vertical-align:middle}
.htable td:first-child{font-family:var(--font-mono);font-weight:700;color:var(--blue);white-space:nowrap}
.host-pill{display:inline-flex;align-items:center;gap:4px;background:color-mix(in srgb,var(--accent) 8%,transparent);border:1px solid color-mix(in srgb,var(--accent) 18%,transparent);color:var(--accent);padding:2px 8px;border-radius:5px;font-family:var(--font-mono);font-size:.72rem;margin:2px}
.host-pill.local{background:color-mix(in srgb,var(--blue) 8%,transparent);border-color:color-mix(in srgb,var(--blue) 18%,transparent);color:var(--blue)}
.htable-actions{display:flex;gap:5px;align-items:center;white-space:nowrap}
.hcomment-badge{font-family:var(--font-mono);font-size:.68rem;color:var(--text3);font-style:italic}
.hosts-toolbar{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:16px;flex-wrap:wrap}
.hosts-search{position:relative;flex:1;min-width:160px;max-width:300px}
.hosts-search input{width:100%;background:var(--bg2);border:1px solid var(--border2);color:var(--text);font-family:var(--font-mono);font-size:.8rem;padding:8px 10px 8px 34px;border-radius:8px;outline:none;transition:border-color .15s}
.hosts-search input:focus{border-color:var(--accent)}
.hosts-search .sico{position:absolute;left:10px;top:50%;transform:translateY(-50%);color:var(--text3);pointer-events:none}
.table-wrap{background:var(--bg2);border:1px solid var(--border);border-radius:var(--radius);overflow:hidden}
.table-wrap table{margin:0}

/* FOOTER */
footer{margin-top:44px;padding-top:18px;border-top:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;font-size:.7rem;color:var(--text3);font-family:var(--font-mono);flex-wrap:wrap;gap:8px}

@media(max-width:640px){
  .vhost-row{grid-template-columns:20px 1fr auto}
  .vactions .btn-sm:not([data-keep]){display:none}
...
<?php if(empty($vhosts)): ?>
  <div class="empty"><div class="e">🗂</div><p>Aucun VirtualHost trouvé dans sites-available/</p></div>
<?php else: ?>
...
<div class="vhost-card <?= $v['enabled']?'':'disabled' ?>"
  ...
  <div class="vdetails" id="d<?= $i ?>">
    <div class="detail-grid">
      <div class="di"><label>ServerName</label><div class="val"><?= $v['server_name']?'<a href="'.htmlspecialchars($url).'" target="_blank">'.htmlspecialchars($v['server_name']).'</a>':'—' ?></div></div>
      <?php if($v['server_alias']): ?><div class="di"><label>ServerAlias</label><div class="val"><?= htmlspecialchars(implode(', ',$v['server_alias'])) ?></div></div><?php endif; ?>
      <div class="di"><label>DocumentRoot</label><div class="val"><?= $v['doc_root']?htmlspecialchars($v['doc_root']):'—' ?></div></div>
      <div class="di"><label>Port</label><div class="val"><?= htmlspecialchars($v['port']) ?></div></div>
      <div class="di"><label>SSL</label><div class="val"><?= $v['ssl']?'✓ Activé':'✗ Non' ?></div></div>
      <div class="di"><label>Fichier</label><div class="val"><?= htmlspecialchars(SITES_AVAILABLE.'/'.$v['file']) ?></div></div>
      <?php if($v['admin_email']): ?><div class="di"><label>ServerAdmin</label><div class="val"><?= htmlspecialchars($v['admin_email']) ?></div></div><?php endif; ?>
      <div class="di"><label>Statut</label><div class="val"><?= $v['enabled']?'<span style="color:var(--accent)">● Activé</span>':'<span style="color:var(--text3)">○ Désactivé</span>' ?></div></div>
    </div>
...
<div class="overlay" id="viewModal" onclick="closeOverlay('viewModal',event)">
  <div class="modal">
    <div class="modal-head"><h2 id="viewTitle">Configuration</h2><button class="btn btn-ghost btn-sm" onclick="closeOverlay('viewModal')">✕</button></div>
...
<div class="overlay" id="createModal" onclick="closeOverlay('createModal',event)">
  <div class="modal wide">
    <div class="modal-head"><h2>＋ Nouveau VirtualHost</h2><button class="btn btn-ghost btn-sm" onclick="closeOverlay('createModal')">✕</button></div>
...
<div class="overlay" id="editModal" onclick="closeOverlay('editModal',event)">
  <div class="modal wide">
    <div class="modal-head"><h2 id="editTitle">✎ Modifier</h2><button class="btn btn-ghost btn-sm" onclick="closeOverlay('editModal')">✕</button></div>
    <div class="modal-body">
...
<div class="overlay" id="deleteModal" onclick="closeOverlay('deleteModal',event)">
  <div class="modal" style="max-width:440px">
    <div class="modal-head"><h2>🗑 Confirmer la suppression</h2><button class="btn btn-ghost btn-sm" onclick="closeOverlay('deleteModal')">✕</button></div>
    <div class="modal-body">
...
<div class="overlay" id="hostAddModal" onclick="closeOverlay('hostAddModal',event)">
  <div class="modal" style="max-width:560px">
    <div class="modal-head"><h2>＋ Ajouter une entrée DNS</h2><button class="btn btn-ghost btn-sm" onclick="closeOverlay('hostAddModal')">✕</button></div>
    <div class="modal-body">
...
<div class="overlay" id="hostEditModal" onclick="closeOverlay('hostEditModal',event)">
  <div class="modal" style="max-width:560px">
    <div class="modal-head"><h2>✎ Modifier une entrée DNS</h2><button class="btn btn-ghost btn-sm" onclick="closeOverlay('hostEditModal')">✕</button></div>
    <div class="modal-body">
...
<div class="overlay" id="hostDeleteModal" onclick="closeOverlay('hostDeleteModal',event)">
  <div class="modal" style="max-width:420px">
    <div class="modal-head"><h2>🗑 Supprimer l'entrée DNS</h2><button class="btn btn-ghost btn-sm" onclick="closeOverlay('hostDeleteModal')">✕</button></div>
    <div class="modal-body">
...
// ── HOSTS RAW ────────────────────────────────────────────────────────────
function openHostsRaw(){openOverlay('hostsRawModal');}

// ── FILTER ──────────────────────────────────────────────────────────────
let activeF = 'all';
function setF(f,btn){activeF=f;document.querySelectorAll('.ftab').forEach(b=>b.classList.remove('active'));btn.classList.add('active');filterV();}
function filterV(){
  const q=document.getElementById('search').value.toLowerCase();
  document.querySelectorAll('.vhost-card').forEach(c=>{
    const en=c.dataset.en==='1',ssl=c.dataset.ssl==='1',qm=!q||c.dataset.q.includes(q);
...
async function viewCfg(file){
  ...
  try{const r=await fetch('?view='+encodeURIComponent(file));document.getElementById('viewCode').value=await r.text();}
  catch(e){document.getElementById('viewCode').value='Erreur : '+e.message;}
}
...
async function openEdit(file){
  ...
  try{const r=await fetch('?edit_raw='+encodeURIComponent(file));document.getElementById('editCode').value=await r.text();}
  catch(e){document.getElementById('editCode').value='Erreur : '+e.message;}
}

// ── CREATE ───────────────────────────────────────────────────────────────
function openCreate(){openOverlay('createModal');}
...
function autoFillFilename(sn){
  ...
  if(fi){ fi.value = clean; updateFilenamePreview(clean); }
}
...
document.addEventListener('DOMContentLoaded',()=>{
  const fi = document.getElementById('guidedFilename');
  if(fi) fi.addEventListener('input',()=>{ filenameManuallyEdited=true; updateFilenamePreview(fi.value); });
});
...
// ── TOAST AUTO-HIDE ──────────────────────────────────────────────────────
setTimeout(()=>{const t=document.querySelector('.toast');if(t){t.style.transition='opacity .4s';t.style.opacity='0';setTimeout(()=>t.remove(),400);}},5000);
</script>
</body>
</html>
